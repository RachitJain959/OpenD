...
actor OpenD {
    ...
    var mapListings = HashMap.HashMap<Principal, Listing>(1, Principal.equal, Principal.hash);
    ...
    public query func getOwnedNFTs(user : Principal) : async [Principal] {
        var userNFTs : List.List<Principal> = switch (mapOwners.get(user)) {
            case null List.nil<Principal>();
            case (?result) result;
        };
    ...
    public query func getListedNFTs() : async [Principal] {
        let ids = Iter.toArray(mapListings.keys());
        return ids;
    };
    ...
    public query func getOriginalOwner(id : Principal) : async Principal {
        var listing : Listing = switch (mapListings.get(id)) {
